...
export class InvitesService {
  ...
  async acceptInvite(id: string, acceptInviteDto: AcceptInviteDto) {
    ...
    try {
      await this.rabbitMQService.emit('invite.accepted', {
        inviteeEmail: invite.email,
        inviteeName: invite.email,
        organizationId: invite.organizationId,
        organizationName: invite.organization.name,
        adminEmail: invite.organization.email,
        role: invite.role,
        acceptedAt: new Date().toISOString(),
      });
    } catch (error: unknown) {
      this.logger.warn(`Failed to emit invite.accepted event: ${(error as Error).message}`);
    }
